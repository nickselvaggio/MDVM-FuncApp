...
def _fetch_device_details(access_token: str, device_id: str) -> dict:
    """Fetch device details including machineTags from Defender API."""
    ...
    try:
        ...
        logging.warning(f"Failed to fetch device details for {device_id}: {exc}")
        return {}


def _reorganize_vulnerabilities_by_hierarchy(vulnerabilities: list, access_token: str = None) -> dict:
    """
    Reorganize vulnerabilities data into hierarchical structure:
    osPlatform -> deviceName -> {device metadata + vulnerabilities{cveId}}
    """
    reorganized = {}
    device_metadata_cache = {}  # Cache to store device-level info including tags
    ...
    for vuln in vulnerabilities:
        ...
        device_id = vuln.get("deviceId", "Unknown")
        ...
        # Create device key for caching
        device_key = f"{os_platform}:{device_name}"
        
        # Initialize platform if not exists
        if os_platform not in reorganized:
            reorganized[os_platform] = {}
        
        # Initialize device if not exists with metadata
        if device_name not in reorganized[os_platform]:
            # Fetch device details including tags if access_token is provided
            machine_tags = []
            if access_token and device_id not in device_metadata_cache:
                device_details = _fetch_device_details(access_token, device_id)
                machine_tags = device_details.get("machineTags", [])
                device_metadata_cache[device_id] = {"machineTags": machine_tags}
            elif device_id in device_metadata_cache:
                machine_tags = device_metadata_cache[device_id].get("machineTags", [])
            
            reorganized[os_platform][device_name] = {
                "deviceId": device_id,
                "deviceName": device_name,
                "tags": machine_tags,
                "rbacGroupName": vuln.get("rbacGroupName", "Unassigned"),
                "osPlatform": os_platform,
                "osVersion": vuln.get("osVersion", "Unknown"),
                "osArchitecture": vuln.get("osArchitecture", "Unknown"),
                "vulnerabilities": {}
            }
        
        # Add vulnerability to this device
        reorganized[os_platform][device_name]["vulnerabilities"][cve_id] = vuln
...
def getMDVMData(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if reorganize and vulnerabilities_data['vulnerabilities']:
            logging.info("Reorganizing vulnerability data and fetching device tags...")
            reorganized_data = _reorganize_vulnerabilities_by_hierarchy(
            ...
            cve_count = sum(
                len(device_data.get("vulnerabilities", {})) 
                for devices in reorganized_data.values() 
